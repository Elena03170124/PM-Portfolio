...
export function PillarCard({ pillar }: { pillar: FuturePillar }) {
  ...
  return (
    <CornerBracketCard className="p-5 sm:p-6">
      <span className="font-mono text-[13px] text-amber">{String(pillar.index).padStart(2, '0')}</span>
      <h3 className="mt-2.5 font-sans text-[15px] font-semibold text-fg leading-snug">{t(pillar.name)}</h3>
